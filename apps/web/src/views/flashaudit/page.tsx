...
const FlashAudit: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // First check source code
    if (result.is_open_source === "1") {
      safeTotal++; // Count verified source code
  
      // Only check these if source code is verified
      const fieldsToCheck = {
        risk: ['owner_change_balance', 'is_honeypot'],
        attention: [
          'external_call',
      ...
      // Check risk items
      fieldsToCheck.risk.forEach(field => {
        if (result[field as keyof TokenSecurityResponse] === "1") {
          riskyTotal++;
          console.log('Risky:', field);
        } else if (result[field as keyof TokenSecurityResponse] === "0") {
          safeTotal++;
          console.log('Safe:', field);
        }
      });
  
      // Check attention items
      fieldsToCheck.attention.forEach(field => {
        if (result[field as keyof TokenSecurityResponse] === "1") {
          attentionTotal++;
      ...
      attentionTotal++; // Count unverified source code as attention
    }
  
    // Add static checks
    safeTotal++; // For "not a gas abuser"
    
    // Add trust list check
